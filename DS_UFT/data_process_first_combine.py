...
base_dir = '/homes/Tianlab/weiming/combine/'  # Data path
# clus_coef_list = ['', '_0.5', '_0.6',  '_0.7', '_0.8', '_0.9']
real_names = [dI for dI in os.listdir(base_dir) if os.path.isdir(pjoin(base_dir, dI))]
combine_aim = 'combine_0.5'
clus_coef = '_0.5'

mkdirs(pjoin('combine_data_process', combine_aim))
print_log = open(pjoin('combine_data_process', combine_aim, combine_aim + '.txt'), 'w')

file_name = pjoin(base_dir, combine_aim, combine_aim + '.fasta')
print(file_name)
...
print('after filtered, there are:\t %d seqs' % len([k for k in length if
                                                    threshold_upper + 1 >= k >= threshold_lower - 1]),
      file=print_log)
    ...
    sns.histplot(length,
             ...
             color='green',
             alpha=0.3, )
pic_save_path = pjoin('combine_data_process', combine_aim, combine_aim + '_dist_all.png')
...
plt.close()
    ...
    sns.histplot(sigma3_length,
             ...
             color='green',
             alpha=0.3, )
pic_save_path = pjoin('combine_data_process', combine_aim, combine_aim + '_dist_3sigma.png')
plt.savefig(pic_save_path)
plt.close()
print_log.close()
