...
def train_esm2(train_iter, net, loss, devices, num_steps, in_model, out_model):

    net.esm2model.load_state_dict(torch.load(in_model))
    print('load model from ', in_model)
    ...
    optim = torch.optim.Adam(params=net.parameters(), lr=1e-5)  # 1e-4 1e-5
    step = 0
...
if __name__ == '__main__':
    ...
    model_base_dir = '/homes/Tianlab/weiming'  # tune_model_0.9 tune_model_formal_0.5
    ...
    in_model = pjoin(model_base_dir, 'tune_model_0.5', pfam_aim, pfam_aim + '_0.5.pth')

    mkdirs(pjoin(model_base_dir, 'tune_model_0.9', pfam_aim))
    out_model = pjoin(model_base_dir, 'tune_model_0.9', pfam_aim, pfam_aim + clus_coef + '.pth')
    ...
    num_steps = int(math.ceil(data_size*50/batch_size/10000)*10000)
    print('data path:\t %s' % (pfam_aim + clus_coef + '.fasta'))
    print('batch size:\t %d' % batch_size)
    print('step num:\t %d' % num_steps)
    print('fine tune all layers')
    train_esm2(train_iter, net, loss, devices, num_steps=num_steps, in_model=in_model, out_model=out_model)